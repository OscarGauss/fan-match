...
const STAKE_AMOUNTS = [0.1, 0.25, 0.5] as const;
...
export default function StakePanel({
  ...
  const decisionLogEl = (
    <div
      className="min-h-0 flex-1 flex flex-col overflow-hidden border-t"
      style={{ borderColor: 'var(--border)', background: 'var(--bg-surface)' }}
    >
      <div
        className="shrink-0 flex items-center border-b px-3 py-2"
          ...
          feedEntries.map((entry) => (
            ...
            >
              <span style={{ color: 'var(--text-dim)', flexShrink: 0 }}>
                [{Math.floor(entry.elapsedMs / 60000)}:
                {String(Math.floor((entry.elapsedMs % 60000) / 1000)).padStart(2, '0')}]
              </span>
