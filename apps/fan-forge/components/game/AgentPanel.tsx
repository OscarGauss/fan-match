...
function teamBorder(t: Team)   { return t === 'red' ? 'var(--red-border)' : 'var(--blue-border)' }
function teamColorRaw(t: Team) { return t === 'red' ? '#ff4d4d'           : '#4d9fff'            }
...
function FundSlider({ value, colorRaw, onChange }: { value: number; colorRaw: string; onChange: (v: number) => void }) {
  ...
  return (
    <div className="relative flex items-center" style={{ height: 24 }}>
      <div className="w-full rounded-full" style={{ height: 3, background: 'var(--bg-surface)', position: 'relative' }}>
        <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', width: `${pct}%`, background: colorRaw, borderRadius: 'inherit' }} />
      </div>
      <div style={{
        position: 'absolute', left: `calc(${pct}% - 7px)`,
        width: 14, height: 14, borderRadius: '50%',
        background: colorRaw, border: '2px solid var(--bg-panel)',
        boxShadow: `0 0 0 2px ${colorRaw}44, 0 2px 4px rgba(0,0,0,0.4)`,
        pointerEvents: 'none',
      }} />
      <input type="range" min={FUND_SLIDER_MIN} max={FUND_SLIDER_MAX} step={FUND_SLIDER_STP}
    ...
    <div style={{ paddingBottom: 2 }}>
      ...
      <div className="flex gap-1.5 mb-2">
        {FUND_AMOUNTS.map(a => {
          ...
          return (
            <button key={a} onClick={() => setValue(a)}
              className="flex-1 rounded py-0.5 text-[10px] transition-all"
              style={{ ...MONO, border: `1px solid ${active ? color : 'var(--border-accent)'}`, background: active ? `${colorRaw}22` : 'transparent', color: active ? color : 'var(--text-muted)' }}>
              {a.toFixed(2)}
            </button>
          )
        })}
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <motion.span key={value.toFixed(2)} initial={{ opacity: 0.5, scale: 0.9 }} animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.1 }} className="w-8 text-right text-[10px] font-bold tabular-nums"
          style={{ ...MONO, color }}>
          {value.toFixed(2)}
        </motion.span>
      </div>
...
function SupportChart({ redUsdc, blueUsdc }: { redUsdc: number; blueUsdc: number }) {
  ...
  const linePath = edge.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ')
  ...
  return (
    <div className="flex flex-1 flex-col gap-1.5 min-h-0">
      ...
      <div className="flex items-center justify-between shrink-0">
        <span className="text-[11px] font-bold" style={{ ...MONO, color: 'var(--red)'  }}>Red {redPct}%</span>
        <span className="text-[9px] uppercase tracking-wider" style={{ color: 'var(--text-dim)' }}>fan support</span>
        <span className="text-[11px] font-bold" style={{ ...MONO, color: 'var(--blue)' }}>Blue {bluePct}%</span>
      </div>

      {/* Chart — fills remaining space */}
      <div className="flex-1 min-h-0" style={{ borderRadius: 6, overflow: 'hidden', background: 'var(--bg-surface)', position: 'relative' }}>
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none"
          ...
          <line x1={0} y1={H / 2} x2={W} y2={H / 2}
            stroke="#ffffff20" strokeWidth={0.8} strokeDasharray="4 4" />
          {/* Data line */}
          <path d={linePath} fill="none" stroke="#ffffff55" strokeWidth={1.5} strokeLinejoin="round" />
          {/* Live dot */}
...
function SupportBar({ redUsdc, blueUsdc }: { redUsdc: number; blueUsdc: number }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="relative flex h-2 w-full overflow-hidden rounded-full" style={{ background: 'var(--bg-surface)' }}>
        <motion.div className="h-full" style={{ background: 'var(--red)' }}
          animate={{ width: `${redFrac * 100}%` }} transition={{ duration: 0.5, ease: 'easeOut' }} />
        <motion.div className="h-full flex-1" style={{ background: 'var(--blue)' }}
          animate={{ width: `${blueFrac * 100}%` }} transition={{ duration: 0.5, ease: 'easeOut' }} />
        <div className="pointer-events-none absolute inset-y-0 left-1/2 w-px"
          style={{ background: 'rgba(255,255,255,0.25)' }} />
      </div>
      <div className="flex justify-between">
        <span className="text-[10px]" style={{ ...MONO, color: 'var(--red)'  }}>{redUsdc.toFixed(2)} USDC</span>
        <span className="text-[10px]" style={{ ...MONO, color: 'var(--blue)' }}>{blueUsdc.toFixed(2)} USDC</span>
      </div>
    </div>
  )
}
...
export default function AgentPanel({
  ...
  useEffect(() => {
    ...
    for (const k of COMPACT_ORDER) { if (cur[k] > prev[k]) fresh.add(k) }
    if (fresh.size > 0) {
    ...
    <div className="flex h-full flex-col overflow-hidden" style={{ background: 'var(--bg-panel)' }}>
      ...
      <div className="flex shrink-0 border-b" style={{ borderColor: 'var(--border)' }}>
        {TAB_ORDER.map(v => {
          ...
          const label = v === 'support'
            ? (userTeam === 'red' ? 'Support Red' : 'Support Blue')
            : 'Agents'
          return (
            <button key={v} onClick={() => onViewChange(v)}
              className="relative flex-1 py-2.5 text-xs font-bold uppercase tracking-widest transition-colors"
              style={{ color: active ? 'var(--text-primary)' : 'var(--text-muted)', background: 'transparent', border: 'none' }}>
              {label}
              {active && (
        ...
        <div className="flex flex-1 flex-col overflow-hidden p-2.5 gap-2">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex flex-col gap-0.5">
              <span className="text-sm font-bold" style={{ ...MONO, color }}>{agentName}</span>
              <span className="text-[10px]" style={{ ...MONO, color: 'var(--text-muted)' }}>
                {truncateAddr(agent.walletAddress)}
              </span>
            </div>
            <motion.div key={agent.usdcReceived}
            ...
            {COMPACT_ORDER.map((key, idx) => {
              ...
              return (
                <span key={key} className="flex items-center gap-1">
                  {idx > 0 && <span style={{ ...MONO, color: 'var(--text-dim)', fontSize: 9 }}>·</span>}
                  <motion.span
                    style={{ ...MONO, fontSize: 9, color: isFlashing ? color : 'var(--text-muted)' }}
          ...
          {agent.activeBoost && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <span className="text-[10px] font-bold" style={{ color }}>⚡ boost active ×1.5</span>
            </motion.div>
          )}
