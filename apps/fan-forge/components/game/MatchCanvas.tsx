...
export default function MatchCanvas({ matchState, onGoal, onFeedEntry }: MatchCanvasProps) {
  ...
  useEffect(() => { msRef.current = matchState },    [matchState])
  ...
  const phys = useRef({
    bx: 400, by: CANVAS_H / 2,
    vx: BALL_SPEED, vy: BALL_SPEED * 0.4,
  ...
  useEffect(() => {
    ...
    if (prev.status !== curr.status) {
      ...
      else if (prev.status === 'grid_event')
        push(null, 'Grid Event ended')
      else if (curr.status === 'finished') {
        const w = curr.score.red > curr.score.blue ? 'red'
                : curr.score.blue > curr.score.red ? 'blue' : null
        push(null, w ? `Match over · ${w === 'red' ? 'AgentRed' : 'AgentBlue'} wins` : 'Match over · Draw')
      }
    }
  ...
  useEffect(() => {
    ...
    function drawField(f: Field) {
      ...
      // Penalty boxes
      const penW = f.w * 0.13; const penH = f.h * 0.52
    ...
    function drawRods(f: Field, ms: MatchState) {
      ROD_DEFS.forEach((def, i) => {
        ...
        ctx.lineWidth = 2
        ctx.beginPath(); ctx.moveTo(rx, f.y); ctx.lineTo(rx, f.y + f.h); ctx.stroke()

        // Boost glow
        if (boosted) {
          ctx.strokeStyle = def.team === 'red'
            ? 'rgba(255,77,77,0.28)' : 'rgba(77,159,255,0.28)'
          ctx.lineWidth = 6
          ctx.beginPath(); ctx.moveTo(rx, f.y); ctx.lineTo(rx, f.y + f.h); ctx.stroke()
        }
    ...
    function tick(time: number) {
      ...
      ROD_DEFS.forEach((def, i) => {
        ...
        p.rodY[i]  = clamp(p.rodY[i] + Math.sign(diff) * Math.min(Math.abs(diff), spd * dt), minY, maxY)
      })
      ...
      // Wall bounce
      if (p.by - BALL_R < f.y)        { p.by = f.y + BALL_R;        p.vy =  Math.abs(p.vy) }
      if (p.by + BALL_R > f.y + f.h)  { p.by = f.y + f.h - BALL_R; p.vy = -Math.abs(p.vy) }
